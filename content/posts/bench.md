...
draft = false
title = 'How to benchmark with Java Microbenchmark Harness (JMH)'
+++

I have been binging performance-related content lately, and the phrase "measure first" comes out a lot. A fond memory of mine of "measuring" is taking a video of an iPhone timer next to our app to prove that the new build is many seconds slower than the previous one. Good old days as a manual QA tester. Now, as I am trying to write a Single Producer Single Consumer queue that works in the nanosecond range, I started reading about micro-benchmarking, which is what we will setup.

The easiest way to measure how fast a function runs is pretty simple. Measure the start, run the function, and measure the end. Substract the difference, and we have the result.
...
Let's run the benchmark with `./gradlew jmh`. Note, if you run above, it will take a fairly long time because of JMH's default settings. You can see the defaults in the terminal after you run JMH:
...
We can see from results below that summing using a primitive allows us to process ~3.5x times more than with autoboxing.
...
And there you go, not too hard to measure! Well, just to start anyway. There are so many things that would affect our measurements (and what we are actually measuring) such as:
- Dead Code Elimination
- Constant Folding
- Cache locality

We also need to explore other modes such as `SampleTime` which would give us tail latencies such as p99.

As a summary, we managed to setup JMH and run a simple benchmark. 
